...
void    cub_texture_allocate(t_cub *cub)
{
	...
	while (cub->game.map_str && *cub->game.map_str)
	{
		...
		else if (ft_strncmp(cub->game.map_str + i, "SO ", 3) == 0 && cub->texture.s_flag == 0)
			init_texture_south(cub, &i);
		else if (ft_strncmp(cub->game.map_str + i, "WE ", 3) == 0 && cub->texture.w_flag == 0)
			init_texture_west(cub, &i);
		else if (ft_strncmp(cub->game.map_str + i, "EA ", 3) == 0 && cub->texture.e_flag == 0)
			init_texture_east(cub, &i);
		else if (ft_strncmp(cub->game.map_str + i, "F ", 2) == 0 && cub->game.map_str[i] != '\0')
			init_texture_floor(cub, &i);
		else if (ft_strncmp(cub->game.map_str + i, "C ", 2) == 0 && cub->game.map_str[i] != '\0')
			init_texture_ceiling(cub, &i);
		else if (cub->game.map_str[i] == '\n' || cub->game.map_str[i] == ' ')
			cub->game.newline_c++;
		else if (cub->game.map_str[i] != '1' && cub->game.map_str[i] != '0' && cub->game.map_str[i] != '\n' )
