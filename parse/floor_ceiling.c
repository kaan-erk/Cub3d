...
void	cub_fc_error(t_cub *cub)
{
	...
	if (get_array_len(temp_floor) != 3 || get_array_len(temp_ceiling) != 3)
	{
		free_split(temp_floor);
		free_split(temp_ceiling);
		exit_free_cub("Error: Floor or Ceiling must have 3 components (R,G,B)!",
			1, cub);
	}
	if (!int_check_fc(temp_floor, temp_ceiling))
	{
		free_split(temp_floor);
		free_split(temp_ceiling);
		exit_free_cub("Error: Floor/Ceiling contains non-numeric characters!",
			1, cub);
	}
	i = 0;
	while (i < 3)
	{
		f_val = ft_atoi(temp_floor[i]);
		c_val = ft_atoi(temp_ceiling[i]);
		if (f_val < 0 || f_val > 255 || c_val < 0 || c_val > 255)
		{
			free_split(temp_floor);
			free_split(temp_ceiling);
			exit_free_cub("Error: RGB values must be between 0 and 255!", 1,
				cub);
		}
		i++;
	}
	cub->texture.floor = (ft_atoi(temp_floor[0]) << 16) | (ft_atoi(temp_floor[1]) << 8) | ft_atoi(temp_floor[2]);
	cub->texture.ceiling = (ft_atoi(temp_ceiling[0]) << 16) | (ft_atoi(temp_ceiling[1]) << 8) | ft_atoi(temp_ceiling[2]);
	free_split(temp_floor);
	free_split(temp_ceiling);
}
